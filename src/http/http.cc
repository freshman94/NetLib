...
void onRequest(const HttpRequest& request, HttpResponse* response)
{
	...
	if (!benchmark) {
		const std::unordered_map<string, string>& headers = request.headers();
	...
	if (request.path() == "/")
	{
		response->setStatusCode(HttpResponse::OK200);
		response->setStatusMessage("OK");
		response->setContentType("text/html");
		response->addHeader("HttpServer", "DJH's");
		response->setBody("<html><head><title>This is title</title></head>"
			"<body><h1>Hello</h1> This is DJH's HttpServer~~</body></html>");
	}
	...
	{
		response->setStatusCode(HttpResponse::NotFound404);
...
int main(int argc, char* argv[])
{
	...
	EventLoop loop;
	HttpServer server(&loop, InetAddress("192.168.6.115", 8000), "dummy", numThreads);
