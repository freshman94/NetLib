...
public:
	Session(EventLoop* loop, const InetAddress& serverAddr,
		const string& name, Client* owner)
		: client_(loop, serverAddr, name),
		owner_(owner), bytesRead_(0),
		bytesWritten_(0), messagesRead_(0)
	{
...
public:
	Client(EventLoop* loop, const InetAddress& serverAddr,
		int blockSize, int sessionCount,
		int timeout, int threadCount)
		: loop_(loop),
		threadPool_(loop, "pingpong-client", threadCount),
	...
	void onConnect()
	{
		if (numConnected_.increAndGet() == sessionCount_)
			LOG_WARN << "all connected";
	}

	void onDisconnect(const TcpConnectionPtr& conn)
	{
		if (numConnected_.decreAndGet() == 0)
		{
